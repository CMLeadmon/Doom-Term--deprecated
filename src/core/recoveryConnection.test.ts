...
const connections: RecoveryConnection[] = [];
...
function fixture() {
  ...
  const connection = new RecoveryConnection({ socket: () => { const socket = new Socket(); sockets.push(socket); return socket; },
...
describe('negotiated recovery connection', () => {
  ...
  it('bounds incoming frames before JSON dispatch and discards malformed data without echoing it', () => {
    const f = fixture(); const socket = f.sockets[0]; socket.open(); socket.negotiate();
    socket.onmessage?.({ data: 'sensitive'.repeat(600000) });
    expect(f.connection.state.status).toBe('incompatible'); expect(f.events).not.toHaveBeenCalled();
  ...
  it('does not leak a heartbeat when a ready-state observer immediately disconnects the socket', () => {
    ...
    const connection = new RecoveryConnection({ socket: () => socket, onMessage: () => undefined,
