...
export type RecoverySocket = DaemonSocket;
interface Options {
  socket(): DaemonSocket;
...
const knownEvents = new Set([
  'AuthResult', 'Protocol', 'Negotiated', 'Incompatible', 'Pong',
  'Telemetry', 'TelemetryUnavailable', 'AgentEvent', 'ArtifactEvent', 'OperationError',
  'AttachResult', 'StreamBegin', 'StreamRecord', 'StreamCaughtUp', 'AttachmentReady', 'StreamUnavailable',
  'SessionListing', 'DirectoryListing',
  'HistoryChunk', 'HistoryComplete', 'HistoryUnavailable',
]);
...
export class DaemonConnection {
  ...
  connect(): void {
    ...
    try {
      ...
      socket.onmessage = event => {
        if (!current()) return;
        try {
          if (typeof event.data !== 'string' || event.data.length > MAX_INBOUND || encoder.encode(event.data).length > MAX_INBOUND) {
            ...
            diagnostics.record({
              ...
              sessionId: null,
              requestId: null,
              reason: 'Incoming frame exceeds maximum inbound size',
            });
            return;
          }
          let message: unknown;
          try {
            ...
            diagnostics.record({
              kind: 'refusal',
              name: 'frame:malformed',
              sessionId: null,
              requestId: null,
              reason: 'Malformed JSON frame dropped',
            });
            return;
          }
          if (!message || typeof message !== 'object' || Array.isArray(message)) {
            ...
            diagnostics.record({
              ...
              sessionId: null,
              requestId: null,
              reason: 'Invalid message frame dropped',
            });
            return;
          }
          ...
          if (typeof name !== 'string') {
            ...
            diagnostics.record({
              kind: 'refusal',
              name: 'frame:invalid_name',
              sessionId: null,
              requestId: null,
              reason: 'Frame missing event name dropped',
            });
            return;
          }
          ...
          else if (error instanceof ProtocolVersionMismatch) this.disconnect('incompatible', 'Invalid or incompatible recovery protocol; update both shells together', false);
          else {
  ...
  private receive(event: string, value: unknown): void {
    ...
    if (event === 'AuthResult') {
      if (typeof data.success !== 'boolean' || this.offeredEpoch) throw new ProtocolVersionMismatch('Invalid authentication transition');
    ...
    if (event === 'Protocol' || event === 'Negotiated') {
      if (data.version !== 2 || typeof data.daemon_epoch !== 'string' || !/^[0-9a-f]{32}$/.test(data.daemon_epoch)
          || (this.challengeSeen && !this.authenticated)) throw new ProtocolVersionMismatch('Incompatible protocol');
      const epoch = data.daemon_epoch;
      if (event === 'Protocol') {
        if (this.offeredEpoch !== null) {
          if (this.offeredEpoch !== epoch) throw new ProtocolVersionMismatch('Daemon epoch changed inside socket');
          return;
        }
        ...
        if (this.offeredEpoch !== epoch || !['negotiating', 'ready'].includes(this.current.status)) throw new ProtocolVersionMismatch('Unoffered protocol');
        if (this.current.status === 'ready') return;
        this.clearHandshake();
    ...
    if (event === 'Incompatible' || legacyEvents.has(event)) throw new ProtocolVersionMismatch('Unnegotiated event');
    if (this.current.status !== 'ready') {
      ...
      diagnostics.record({
        ...
        sessionId: null,
        requestId: null,
        reason: `Ignored unready event ${event}`,
      });
      return;
    }
    if (!knownEvents.has(event) && !event.endsWith('Result')) {
      diagnostics.count('unknownEvents');
      diagnostics.record({
        kind: 'event',
        name: 'event:unknown',
