...
import type { DaemonSocket } from '../core/daemonConnection';
...
export class TestDaemonSocket implements DaemonSocket {
  readyState = 0;
  bufferedAmount = 0;
...
export function daemonFixture(): { client: PtyClient; sockets: TestDaemonSocket[] } {
  const sockets: TestDaemonSocket[] = [];
  const client = new PtyClient({ socket: () => { const socket = new TestDaemonSocket(sockets.length + 1); sockets.push(socket); return socket; } });
  return { client, sockets };
}
export const recoveryFixture = daemonFixture;
export const TestRecoverySocket = TestDaemonSocket;
