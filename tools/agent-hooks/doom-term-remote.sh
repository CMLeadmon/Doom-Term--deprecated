...
if [ -z "$DOOM_TERM_BOOTSTRAPPED" ]; then export DOOM_TERM_BOOTSTRAPPED=1; __doom_remote() { __db=$(git --no-optional-locks rev-parse --abbrev-ref HEAD 2>/dev/null); __dj=$(printf '{"v":1,"host":"%s","user":"%s","shell":"%s","cwd":"%s","branch":"%s"}' "$(hostname -s 2>/dev/null)" "$USER" "$(basename "${SHELL:-sh}")" "$PWD" "$__db" | base64 | tr -d '\n'); printf '\033]1337;SetUserVar=doomterm=%s\007' "$__dj"; }; PROMPT_COMMAND="__doom_remote${PROMPT_COMMAND:+; $PROMPT_COMMAND}"; fi
