...
2. **`backend/`**:
   - Standalone Tokio daemon listening on `127.0.0.1:1421` by default, overridable with `DOOM_PORT`. Serves both WebSocket PTY clients and HTTP `POST /hook/:agent` events, plus `GET /health` for identity. The desktop shell moves the daemon to a free port when something that is not a Doom Term daemon already holds the default, and tells the webview which port to dial via `__DOOM_TERM_DAEMON_PORT__`; nothing in the frontend may hardcode the port.
